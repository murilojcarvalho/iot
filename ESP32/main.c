 ...
 */
#define DEBUG 1
#define CONFIG_WIFI_SSID        "9 de julho_EXT"
#define CONFIG_WIFI_PASSWORD    "support0"  

#define BUTTON	GPIO_NUM_17 
#define TOPICO_TEMPERATURA      "DHT11 temperatura"
#define TOPICO_UMIDADE          "DHT11 umidade"
...
static void mqtt_app_start( void )
{
    ...
    const esp_mqtt_client_config_t mqtt_cfg = {
  		.uri =  "mqtt://192.168.15.39:1883",  //192.168.15.14 -> notebook                            
    ...
    for(;;) 
	  {
        ...
        sprintf(buff_temp, "%d C", DHT11_read().temperature);
        sprintf(buff_umid, "%d%%", DHT11_read().humidity);
        
		if((buff_temp[0] == 45) || buff_umid[0] == 45) //45 é '-' em ascii 
			return;
